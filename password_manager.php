...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    if (isset($_POST['add_password'])) {
        ...
        // 查詢資料庫以檢查是否已存在相同的密碼
        $check_sql = "SELECT COUNT(*) AS count FROM passwordmanage WHERE username = ? AND encrypted_password = ?";
        ...
        $check_params = array($username, $encrypted_data);
        ...
        if ($check_row['count'] > 0) {
            echo "<script>alert('警告：該密碼已存在於其他網站！');</script>";
        } else {
